...
if(isset($_GET['reject'])){
    ...
    $fetch = "SELECT ap_date from appointments where ap_id = '$ap_id'";
    $linkfetch = mysqli_query($conn, $fetch);
    $fetchdate = mysqli_fetch_assoc($linkfetch);
    $thedate = $fetchdate['ap_date'];
    $message = 'The payment to the appointments that was set for '. $thedate . '';     
    $reg = "INSERT INTO notifications(sender, title, message1, phone, reciever) VALUES ('".$_SESSION['name']."', 'Payment for appointment has been halted', '$message', '".$_SESSION['phone']."', '".$_SESSION['phone2']."')";
    $rest = mysqli_query($conn, $reg);
    checkSQL($conn, $rest);
